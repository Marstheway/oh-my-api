...
// 只索引同时含有字符串 id 与数值型 context_length 的记录。
...
// 规则：
//  1. 若当前 group（alias 解析后）有配置覆盖，直接返回配置值（覆盖语义，不展开叶子）
//  2. 若当前 group 无配置，递归收集子节点的 context_length：
//     - 子 group 有配置则用配置值，无配置则继续展开
//     - 叶子节点从 catalogIndex 查询
//  3. 多个有效值取最小；0 个有效值返回 nil（省略字段）
func ResolveContextLength(userModel string, r *model.Resolver, catalogIdx catalogContextIndex) *int {
	...
	candidates := r.CollectChildContextLengths(finalGroup, catalogIdx.LookupContextLength)
